...
describe('P1-5: Bash exit code → status mapping', () => {
  it('exit 0 → status:success, isError:false', async () => {
    ...
    const out = await t.execute({ command: 'true' }, ctx()) as typeof ToolResult & { status?: string; exitCode?: number }
  ...
  it('exit 1 (default) → status:failed, isError:true', async () => {
    const t = new BashTool()
    const out = await t.execute({ command: 'false' }, ctx()) as typeof ToolResult & { status?: string; exitCode?: number }
  ...
  it('exit 2 with no allow-list → status:failed', async () => {
    const t = new BashTool()
    const out = await t.execute({ command: 'exit 2' }, ctx()) as typeof ToolResult & { status?: string; exitCode?: number }
    expect(out.isError).toBe(true)
    expect(out.status).toBe('failed')
  ...
  it('exit code in acceptable_exit_codes → status:success, isError:false', async () => {
    ...
    const out = await t.execute(
      ...
      ctx(),
    ) as typeof ToolResult & { status?: string; exitCode?: number; isError?: boolean }
    expect(out.isError).toBe(false)
    expect(out.status).toBe('success')
    expect(out.exitCode).toBe(1)
  })
  ...
  it('non-zero exit NOT in acceptable_exit_codes → still failed', async () => {
    ...
    const out = await t.execute(
      ...
      ctx(),
    ) as typeof ToolResult & { status?: string; exitCode?: number }
    expect(out.isError).toBe(true)
    expect(out.status).toBe('failed')
    expect(out.exitCode).toBe(2)
  })
  ...
  it('malformed acceptable_exit_codes falls back to [0]', async () => {
    ...
    const out = await t.execute({ command: 'exit 1', acceptable_exit_codes: 'oops' }, ctx()) as typeof ToolResult & { status?: string }
    expect(out.status).toBe('failed')
  })
  ...
  it('grep -l with no matches exits 1 — allow-list treats as success', async () => {
    ...
    const out = await t.execute(
      ...
      ctx(),
    ) as typeof ToolResult & { status?: string; isError?: boolean }
    expect(out.isError).toBe(false)
    expect(out.status).toBe('success')
  })
})
