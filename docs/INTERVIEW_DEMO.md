...
| TurnOutcome (canonical result shape) | **Planned** — type defined (`src/core/runtime/turnOutcome.ts`); coordinator still returns legacy TurnResult. Migration is P1. | — | — |
| CriterionEvidence (per-criterion proof) | **Planned** — type + store defined (`src/core/runtime/criterionEvidence.ts`); not yet wired into TaskPlan complete(). | — | — |
...
| Fallback attribution | **Fully wired** | `src/core/runtime/coordinator.ts` (callLLM wrapper) | `MODEL_ATTEMPT_*` events + `onProviderError` hook in `modelGateway.ts` |
